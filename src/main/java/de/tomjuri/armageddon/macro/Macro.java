...
public class Macro {
    ...
    public static void onTick(TickEvent.ClientTickEvent event) {
        ...
        switch (state) {
            ...
            case MOUNT_DILLO:
                KeyBindUtil.rightClick();
                timer.startTimer(ArmageddonConfig.delayAfterMountingDillo);
                break;
            case MINE:
                KeyBindUtil.jump();
                ...
                timer.startTimer(ArmageddonConfig.swipeTime);
                break;
            case SWITCH_TO_ROD_AGAIN:
                Ref.player().inventory.currentItem = ArmageddonConfig.rodSlot - 1;
                timer.startTimer(101);
                break;
            case DESUMMON_DILLO:
                KeyBindUtil.rightClick();
                timer.startTimer(101);
            ...
            case LOOK_AT_BLOCK:
                ...
                timer.startTimer(ArmageddonConfig.lookAtBlockTime);
                break;
            case TELEPORT:
                KeyBindUtil.rightClick();
                timer.startTimer(ArmageddonConfig.delayAfterTeleporting);
                break;
            case UNSNEAK:
