...
object Auth {
    fun auth() {
        ...
        val realData = "${getHWID()}|${System.currentTimeMillis()}|${mc.session.playerID}".toByteArray()
        ...
        val fakeAndRealData = List(10) {
            if (it == 4) CryptoUtil.rsaEncrypt(realData, publicKey)
            else CryptoUtil.rsaEncrypt(generateRandomString(realData.size).toByteArray(), publicKey)
        }
        ...
        OkHttpClient().newCall(request).execute().use { response ->
            ...
            if (responseBody != null) {
                val decoded = CryptoUtil.rsaDecrypt(responseBody.string(), privateKey)
                val split = decoded.split("\\|".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                if (System.currentTimeMillis() - split[2].toLong() < 10000 && "success" == split[0] && getHWID() == split[1]) {
